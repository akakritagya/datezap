...
### Navigation
- Casing-deep header bar, full-bleed edge-to-edge with a 1px panel-line bottom border. Items sit directly on that bar with no per-item box or fill — brand, route links, "API docs", and the Devnagari toggle are bare text/icon groups, distinguished only by color and the active-route underline, not chip borders. Brand mark (pilot-dot + uppercase Big Shoulders wordmark + muted tagline) sits top-left; the Devnagari toggle sits top-right on the same row. Route links + "API docs" group into a second, centered row below. Route links are uppercase Big Shoulders; active route is amber text with a 2px amber underline bar offset below the link; inactive is muted with an ivory hover. No separate mobile nav pattern — the two rows wrap via flexbox.
